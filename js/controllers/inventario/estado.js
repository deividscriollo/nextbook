...
app.controller('editItemEstado', function () {});
app.controller('deleteEstado', function () {});

app.controller('estadoCtrl', function ($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage) {
  ...
  $scope.addititem = function (event) {
    ...
    $mdDialog.show({
      ...
      controller: 'addItemEstado',
      controllerAs: 'ctrl',
      focusOnOpen: false,
      targetEvent: event,
      templateUrl: 'view/dashboardempresa/inventario/modales/crear_estado.html',
  ...
  $scope.eddititem = function (dessert, event) {
    ...
    $mdDialog.show({
      ...
      controller: function editItemEstado($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage, items) {
  ...
  $scope.deleteitem = function (dessert, event) {
    $mdDialog.show({
      clickOutsideToClose: true,     
      locals: {items: dessert},
      controller: function deleteEstado($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage, items) {
  ...
  $scope.getDesserts = function () {
    $scope.promise = serviciosinventario.get_estado().get($scope.query, success).$promise;
  };
...
app.controller('addItemEstado', function ($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage) {
  // codigo grupo
  serviciosinventario.codigo_estado().get().$promise.then(function(data) {
    $scope.data = {
      codigo: data.codigo,
    }
  });
  ...
  this.cancel = $mdDialog.cancel
  $scope.guardar_estado = function() {
    serviciosinventario.add_estado().save($scope.data).$promise.then(function(data) {
      if(data.respuesta == true) {
          $mdDialog.show(
            $mdDialog.alert()
            .parent(angular.element(document.querySelector('#dialogContainer')))
