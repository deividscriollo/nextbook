...
app.controller('editItemGrupo', function () {});
app.controller('deleteGrupo', function () {});

app.controller('grupoCtrl', function ($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage) {
  ...
  $scope.addititem = function (event) {
    ...
    $mdDialog.show({
      ...
      focusOnOpen: false,
      targetEvent: event,
      templateUrl: 'view/dashboardempresa/inventario/modales/crear_grupo.html',
  ...
  $scope.eddititem = function (dessert, event) {
    ...
    $mdDialog.show({
      ...
      controller: function editItemGrupo($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage, items) {
  ...
  $scope.deleteitem = function (dessert, event) {
    $mdDialog.show({
      clickOutsideToClose: true,     
      locals: {items: dessert},
      controller: function deleteGrupo($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage, items) {
  ...
  $scope.getDesserts = function () {
    $scope.promise = serviciosinventario.get_grupos().get($scope.query, success).$promise;
  };
...
app.controller('addItemGrupo', function ($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage) {
  // codigo grupo
  serviciosinventario.codigo_grupo().get().$promise.then(function(data) {
    $scope.data = {
      codigo: data.codigo,
    }
  });
  ...
  this.cancel = $mdDialog.cancel
  $scope.guardar_grupo = function() {
    serviciosinventario.add_grupo().save($scope.data).$promise.then(function(data) {
      if(data.respuesta == true) {
          $mdDialog.show(
            $mdDialog.alert()
            .parent(angular.element(document.querySelector('#dialogContainer')))
