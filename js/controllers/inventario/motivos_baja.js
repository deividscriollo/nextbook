...
app.controller('editItemMotivos', function () {});
app.controller('deleteMotivos', function () {});

app.controller('motivosCtrl', function ($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage) {
  ...
  $scope.addititem = function (event) {
    ...
    $mdDialog.show({
      ...
      controller: 'addItemMotivos',
      controllerAs: 'ctrl',
      focusOnOpen: false,
      targetEvent: event,
      templateUrl: 'view/dashboardempresa/inventario/modales/crear_motivos.html',
  ...
  $scope.eddititem = function (dessert, event) {
    ...
    $mdDialog.show({
      ...
      controller: function editItemMotivos($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage, items) {
  ...
  $scope.deleteitem = function (dessert, event) {
    $mdDialog.show({
      clickOutsideToClose: true,     
      locals: {items: dessert},
      controller: function deleteMotivos($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage, items) {
  ...
  $scope.getDesserts = function () {
    $scope.promise = serviciosinventario.get_motivos().get($scope.query, success).$promise;
  };
...
app.controller('addItemMotivos', function ($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage) {
  // codigo grupo
  serviciosinventario.codigo_motivos().get().$promise.then(function(data) {
    $scope.data = {
      codigo: data.codigo,
    }
  });
  ...
  this.cancel = $mdDialog.cancel
  $scope.guardar_motivos = function() {
    serviciosinventario.add_motivos().save($scope.data).$promise.then(function(data) {
      if(data.respuesta == true) {
          $mdDialog.show(
            $mdDialog.alert()
            .parent(angular.element(document.querySelector('#dialogContainer')))
