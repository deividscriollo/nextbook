...
app.controller('editItemUbicacion', function () {});
app.controller('deleteUbicacion', function () {});

app.controller('ubicacionCtrl', function ($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage) {
  ...
  $scope.addititem = function (event) {
    ...
    $mdDialog.show({
      ...
      controller: 'addItemUbicacion',
      controllerAs: 'ctrl',
      focusOnOpen: false,
      targetEvent: event,
      templateUrl: 'view/dashboardempresa/inventario/modales/crear_ubicacion.html',
  ...
  $scope.eddititem = function (dessert, event) {
    ...
    $mdDialog.show({
      ...
      controller: function editItemUbicacion($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage, items) {
      ...
      templateUrl: 'view/dashboardempresa/inventario/modales/modificar_ubicacion.html', 
      controllerAs: 'ctrl',
      focusOnOpen: false,
      targetEvent: event,
    }).then($scope.getDesserts);
  ...
  $scope.deleteitem = function (dessert, event) {
    $mdDialog.show({
      clickOutsideToClose: true,     
      locals: {items: dessert},
      controller: function deleteUbicacion($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage, items) {
      ...
      templateUrl: 'view/dashboardempresa/inventario/modales/eliminar_ubicacion.html', 
      controllerAs: 'ctrl',
      focusOnOpen: false,
      targetEvent: event,
    }).then($scope.getDesserts);
  ...
  $scope.getDesserts = function () {
    $scope.promise = serviciosinventario.get_ubicacion().get($scope.query, success).$promise;
  };
...
app.controller('addItemUbicacion', function ($mdDialog, $scope, serviciosinventario, servicios, $timeout, $localStorage) {
  // codigo grupo
  serviciosinventario.codigo_ubicacion().get().$promise.then(function(data) {
    $scope.data = {
      codigo: data.codigo,
    }
  });
  ...
  $scope.guardar_ubicacion = function() {
    serviciosinventario.add_ubicacion().save($scope.data).$promise.then(function(data) {
      if(data.respuesta == true) {
          $mdDialog.show(
            $mdDialog.alert()
            .parent(angular.element(document.querySelector('#dialogContainer')))
