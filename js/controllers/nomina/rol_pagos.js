...
app.controller('rol_pagosCtrl', function ($mdDialog, $scope, serviciosnomina, servicios, $timeout, $localStorage, $location) {
  ...
  app.controller('searchItemEmpleado', function ($mdDialog, $scope, serviciosnomina, servicios, $timeout, $localStorage) {
    $scope.selected = [];
    ...
    $scope.iditem =  function (data) {
      serviciosnomina.get_datos_empleados().send({id_empleado:data.id}).$promise.then(function(data) {
        $scope.data = {}; 
        MyService = data;
        // $scope.data.data.nombres_completos = data.respuesta.nombres_completos;
     }); 
